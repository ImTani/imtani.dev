...
Working notes for the simulation substrate. **No visual decisions are made
here** — the art direction is unresolved. Anything visible in `index.html` or
`src/render/debug.ts` is a test harness and is meant to be thrown away.
